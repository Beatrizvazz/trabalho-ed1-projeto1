...
AbdTree leitura_geo(FILE **ssvg1, FILE *arqgeo, int *instru, int * formcriadas, int *id, float *xcent, float *ycent) {
    ...
    while (fgets(str, 500, arqgeo)) {
        ...
        if (strcmp(tipo, "c ") == 0) {
            ...
            *id=i;
            CIRCULO c = cria_circulo(i, x, y, r, cb, cp);
            if(contadorforms==1){
        ...
        if (strcmp(tipo, "r ") == 0) {
            ...
            *id=i;
            RETANGULO r = cria_retangulo(i, x, y, w, h, cb, cp);
            if(contadorforms==1){
...
AbdTree processa_geo(const char* pathgeo, const char* dirsaida, const char* nomegeo, int *instru, int *formcriadas, int *id, float *xcent, float *ycent) {
    ...
    AbdTree a1=leitura_geo(&ssvg1, arqgeo, instru, formcriadas, id, xcent, ycent);
